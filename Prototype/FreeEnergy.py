...
if __name__ == "__main__":
    ...
    operatorlist = ['StressXX_VolScl.Real','StressYY_VolScl.Real','StressZZ_VolScl.Real']
    Clock = True
    ...
    for i in range(len(filelist)):
        if Clock:
            C = param_array[i,args.keyloc[Cindex]]*np.sqrt(args.N)
        #get list of stuff
        operator,data,warm =  GetOperators(filelist[i],operatorlist,'ChemicalPotential.Real')
        Farray,P = F(data[:,1:],data[:,0],args.N,C)
        nsamples,bounds_F,mean_F,semcc_F,kappa_F,unbiasedvar_F,autocor_F = doStats(np.array([]), Farray)
        nsamples,bounds_P,mean_P,semcc_P,kappa_P,unbiasedvar_P,autocor_P = doStats(np.array([]), P)
        nsamples,bounds_mu,mean_mu,semcc_mu,kappa_mu,unbiasedvar_mu,autocor_mu = doStats(np.array([]), data[:,0])
